    ...
    if (file && (file.startsWith("http://") || file.startsWith("https://"))) {
      try {
        ...
        if (fileUrl.origin !== window.location.origin) {
          ...
          file = `${basePath}/file?l=${encodeURIComponent(file)}`;
        }
      } catch (e) {}
    }
    file = URL.parse(file, window.location.href)?.href ?? file.replaceAll("#", "%23");
...
PDFPrintServiceFactory.initGlobals(PDFViewerApplication);
{
  ...
  var validateFileURL = function (file) {
    ...
    // Allowed cross-origin PDFs for external URL viewing
    return;
  };
